...
class IPAdapter(torch.nn.Module):
    ...
    def forward(self, noisy_latents, timesteps, encoder_hidden_states, image_embeds):
        ...
        q1 = self.query(ip_tokens)
        k1 = self.key(encoder_hidden_states)
        v1 = self.value(encoder_hidden_states)
        ...
        #print(torch.bmm(attn_weights, v1).shape)
        #print(attn_weights.shape)
        ip_tokens_2 = torch.bmm(attn_weights, v1)
        ip_tokens_real = self.a * ip_tokens + (1 - self.a) * ip_tokens_2

        encoder_hidden_states = torch.cat([encoder_hidden_states, ip_tokens_real], dim=1)
...
def main():
    ...
    state_dict = torch.load("IPAdapter/models/ip-adapter-plus_sd15.bin", map_location="cpu")
